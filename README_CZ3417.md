...
**修复**：`_parse_flight_objects` 现在读 `codeShare` 与 `mainCarrier`，丢弃共享行
（仅当实际承运航班本身也在报文里时才丢，避免万一共享号是唯一来源时丢数据）。
修复后同一份报文解析出 **40 个真实航班**（06:40→23:20，符合成都→广州一天的班次量），
且 40 行的起降时刻全部完整。页面页脚也注明"只列实际承运航班"。
