...
### 追加：解析路线的优先级（2026-09-19）

修完上一条，10:11 那轮又出问题：**147 条记录、全部没有起降时刻，共享号也回来了**
（航班数从 40 跳到 147）。第一反应是"这轮报文比较薄"，但实测那份报文
**386 个 `airCode`、386 个 `depTime`、386 个 `codeShare`，结构完全正常** —— 不是报文的锅。

真因在 `_extract_qunar_flights` 的路线顺序：

```
旧：data 能当 JSON 解析 -> 通用解析器 extract_records（只抽航班号 + 价格）
    data 解析不了        -> binfo 专用解析器（带时刻/机场/codeShare 归一）
新：永远先走 binfo 专用解析器，一条都解析不出来时才退回通用解析
```

响应内容每轮都不一样，命中"clean JSON 变体"的那一轮就会掉进只认航班号+价格的通用路线，
于是时刻全丢、共享号全部当独立航班。同一份报文两种路线的对比：
...
**配套修复**：`report.py: backfill_times` 改取"历史**众数**"而非"最近一次非空值" ——
起降时刻是静态属性，掺假解析出的错误时刻（3U8729 曾出现一次 16:05/天府，
正解 15:05/双流有上百次样本）不该被当成最新值一直传下去。

### 追加：共享号"学习缓存"（2026-09-19）

路线修好后仍有残留：某些轮次的报文里共享行的 `codeShare` / `mainCarrier` 字段**被掺假
直接抹掉了**，光看报文认不出"3U1168 其实就是 CZ3438"，页面又会冒出假航班
（10:21 那轮残留 3 个）。

因此增加 `user_data/qunar/aliases.json`：在带标记的轮次里把"共享号 -> 实际承运号"
学下来（实测一轮学到 89 条），没带标记的轮次查表归一。

| 场景 | 归一前残留共享号 | 归一后 |
| --- | --- | --- |
| 带标记报文（学习） | 0 | 0（并写入 89 条映射） |
| 没带标记报文（查表） | 3 | **0** |

单测为避免依赖本机状态，把该缓存指向临时文件（`tests/test_flights.py` 顶部）。
